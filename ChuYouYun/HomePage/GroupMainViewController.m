...
- (void)addNav {
    ...
    //添加分类的按钮
    UIButton *SortButton = [[UIButton alloc] initWithFrame:CGRectMake(MainScreenWidth - 60, 20, 40, 40)];
...
//    titleScrollView.delegate = self;
    ...
    for (int i = 0; i < _titleArray.count ; i ++) {
        ...
        [_titleButton setTitleColor:[UIColor colorWithHexString:@"#888888"] forState:UIControlStateNormal];
    ...
    _titleScrollView.contentSize = CGSizeMake(buttonX + 20, 40);
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (int i = 0; i < _buttonsArray.count; i++) {
        ...
        button.tag = 100 + i;
        [self.buttonsArray[i] setTitleColor:[UIColor colorWithHexString:@"#888888"] forState:UIControlStateNormal];
        if (i == index) {
            isScrollAgain = NO;
            [self change:button];
        }
