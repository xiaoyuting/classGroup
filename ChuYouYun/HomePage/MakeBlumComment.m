...
@implementation MakeBlumComment
...
- (void)commitBtn
{
    ...
    {
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:@"请输入点评内容" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alter show];
        return;
    }
    else{
        ...
        [dic setValue:@"3" forKey:@"score"];
        
        [manager AddReviews:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
            NSLog(@"____%@",responseObject);
            if ([responseObject[@"data"] isEqual:[NSNull null]]) {
                NSString *msg = responseObject[@"msg"];
                UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:msg delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alter show];
                [self.navigationController popViewControllerAnimated:YES];
            } else {
                UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:@"点评成功" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alter show];
                [self.navigationController popViewControllerAnimated:YES];
            }
            
        } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
            UIAlertView *alter = [[UIAlertView alloc] initWithTitle:nil message:@"点评失败，请重试" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alter show];
        }];
