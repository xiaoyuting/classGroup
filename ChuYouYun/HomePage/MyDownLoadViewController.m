...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([model.fileName rangeOfString:@"."].location == NSNotFound) {
        ...
        if ([fileManager fileExistsAtPath:[HHHJJ stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",model.fileName]]]) {
            NSString *GGG = [NSString stringWithFormat:@"%@/%@",HHHJJ,model.fileName];
            NSLog(@"----%@",GGG);
            NSString * tttt = [NSString  stringWithFormat:@"%@.mp4",GGG];
            NSLog(@"%@",tttt);
            NSLog(@"%@",[DiskSpaceTool folderSizeAtPath:tttt]);
            BigWindCar_playViewController *plav = [[BigWindCar_playViewController alloc]initWithPath:tttt withName:[NSString stringWithFormat:@"%@",model.fileName]];
...
//        [MBProgressHUD showSuccess:@"文库暂时不能打开" toView:self.view];
