...
@interface MenuViewController ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate> {
    BOOL flag;
    BOOL isPlaying;
    UIImage  *image;
    AVPlayer *player;
...
- (void)addZGView {
    ...
    //添加答案解析的界面
    UIView *ZGJXView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_photosView.frame), MainScreenWidth, 200)];
    ZGJXView.backgroundColor = [UIColor whiteColor];
...
- (void)ZGJXIntroductionText:(NSString *)text {
    ...
    _ZGJXView.frame = CGRectMake(0, CGRectGetMaxY(_photosView.frame), MainScreenWidth, labelSize.size.height + 20);
...
- (void)upButton {
    ...
    if ([_typeStr integerValue] == 1) {//当前为单选题
        ...
        if (_TKAnswerArray.count >= _Number && _TKAnswerArray.count > 0) {
...
- (void)TKViewApperFromCPBGView {
    ...
    //添加解析视图
    NSLog(@"%@  %@",_TKRightAnswer,_myTKAnswerArray);
...
- (void)ZGViewApperFormCPBGView {
    ...
    _textView.frame = CGRectMake(10, 10, MainScreenWidth - 20, 130);
...
- (void)ZGViewApperWithNext {
    ...
    _ZGTextStr = _textView.text;
    //输入框以及上传图片的视图
    if (_ZGAnswerArray.count && _ZGAnswerArray[_Number + 1] != nil ) {
        _textView.text = _ZGAnswerArray[_Number + 1];
    } else {
        _textView.text = @"";
    }
