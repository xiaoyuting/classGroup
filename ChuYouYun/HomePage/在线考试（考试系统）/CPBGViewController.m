...
- (void)getRigthAnswer {
    ...
    //初始化数组
    _DXRightAnswer = [NSMutableArray array];
    _DDXRightAnswer = [NSMutableArray array];
    _TKRightAnswer = [NSMutableArray array];
    _PDRightAnswer = [NSMutableArray array];
...
- (void)addDDXView {
    ...
    for (int i = 0 ; i < Count ; i ++) {
        ...
        NSString *d5 = [myStr stringByReplacingOccurrencesOfString:@"-" withString:@","];
        NSLog(@"d = %@", d5);
        
        if (![rightStr isEqualToString:d5]) {//说明答案不正确
            button.backgroundColor = FalseColor;
        }
...
- (void)addTKView {
    ...
    for (int i = 0 ; i < Count ; i ++) {
        ...
        NSString *myStr1 = _TKAnswerArray[i];
        NSString *rightStr = _TKRightAnswer[i];
        
        NSLog(@"%@  %@",myStr1,rightStr);
        NSLog(@"%@  %@",_TKAnswerArray,_TKRightAnswer);
//        NSString *myStr2 =[myStr1 stringByReplacingOccurrencesOfString:@" "withString:@""];
//        NSLog(@"mystr2--%@",myStr2);
        if (![rightStr isEqualToString:myStr1]) {//说明答案不正确
            button.backgroundColor = FalseColor;
        } else {
            button.backgroundColor = TrueColor;
        }
...
//    DTVC.imagePhotoArray = _imagePhotoArray;
//    [self.navigationController pushViewController:DTVC animated:YES];
    ...
    menuVc.formWhere = @"123";
    menuVc.dataSource = _dataSource;
    menuVc.formType = typeStr;
    menuVc.formCPNumber = number;
    menuVc.myDXAnswerArray = _DXAnswerArray;
    menuVc.myDDXAnswerArray = _DDXAnswerArray;
    menuVc.myTKAnswerArray = _TKAnswerArray;
    menuVc.myPDAnswerArray = _PDAnswerArray;
    menuVc.myZGAnswerArray = _ZGAnserArray;
    menuVc.imagePhotoArray = _imagePhotoArray;
    menuVc.myZGUpImageArray = _ZGUpImageArray;
    menuVc.myZGUpImageIDArray = _ZGUpImageIDArray;
...
- (void)NetWorkData {
    ...
    if (_timeOut < 60) {
        timeStr = [NSString stringWithFormat:@"0时0分%ld秒",_timeOut];
    } else if (_timeOut % 60 == 0) {
        timeStr = [NSString stringWithFormat:@"0时%ld分0秒",_timeOut / 60];
    } else if (_timeOut > 3600) {
        timeStr = @"1时";
    }
    [dic setObject:timeStr forKey:@"total_date"];
...
- (void)getAnswerStr {
    ...
    for (int i = 0 ; i < _DDXAnswerArray.count ; i ++) {
        NSString *oldStr = _DDXAnswerArray[i];
        NSString *newStr = nil;
        if ([oldStr rangeOfString:@"-"].location !=NSNotFound ) {
            newStr = [oldStr stringByReplacingOccurrencesOfString:@"-"withString:@","];
            [_DDXAnswerArray replaceObjectAtIndex:i withObject:newStr];
        }
    }
    ...
    if (_imageIDArray.count == _SubjectiveArray.count) {//说明是慢的
        ...
        for (int  i = 0 ; i < _SubjectiveArray.count ; i ++) {
            NSString *IDStr = nil;
            if (i < _SubjectiveArray.count) {
                if (i < _imageIDArray.count) {
                     IDStr = [NSString stringWithFormat:@"%@",_imageIDArray[i]];
                }
            }
...
//        NSArray *imageIDArray = _ZGUpImageIDArray[i];
//        if (imageIDArray.count) {
//            _PDStr = [NSString stringWithFormat:@"%@+%@+%@",_SubjectiveIDArray[i],_ZGAnserArray[i],imageIDArray[0]];
//        }
