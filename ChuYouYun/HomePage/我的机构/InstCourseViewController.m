...
//    [self creatMenu];
    [self addTableView];
    [self netWorkGetArrange:_timeSp];
...
//    [backButton setTitleEdgeInsets:UIEdgeInsetsMake(0,-10.0 , 0.0, 0)];
    ...
    [dateButton setTitleColor:BasidColor forState:UIControlStateNormal];
...
- (void)netWorkGetArrange:(NSString *)timeSp {
    ...
    [dic setObject:timeSp forKey:@"timespan"];
    ...
    [manager BigWinCar_getArrange:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 1) {
            ...
            NSArray *allTimeArray = responseObject[@"data"];
            
            for (int i = 0 ; i < allTimeArray.count ; i ++) {
                
                NSString *statTime = allTimeArray[i][@"start_time"];
                
                NSString *startTime = [Passport formatterTime:statTime];
//                NSString *endTime = [Passport formatterTime:dict[@"end_time"]];
                ...
                NSString *hour = [startHour substringWithRange:NSMakeRange(0, 2)];
                
                NSLog(@"%@",hour);
                
                if ([hour integerValue] <= 12) {
                    [_mornArray addObject:allTimeArray[i]];
                } else {
                    [_afterArray addObject:allTimeArray[i]];
                }

            }
            
            NSLog(@"%@  %@",_mornArray,_afterArray);
            
            for (int i = 0 ; i < 2 ; i ++) {
                if (i == 0) {
                    NSLog(@"%@",_dataArray);
                    [_dataArray addObject:_mornArray];
                }
                
                if (i == 1) {
                    [_dataArray addObject:_afterArray];
                }
            }
