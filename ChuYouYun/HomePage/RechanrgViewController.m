...
@interface RechanrgViewController ()<UIWebViewDelegate>
...
@property (assign ,nonatomic)BOOL isArgree;
...
@implementation RechanrgViewController
...
//    HFLabel.backgroundColor = [UIColor blueColor];
    ...
    //添加同意按钮
    _isArgree = NO;
    UIButton *argreeButton = [[UIButton alloc] initWithFrame:CGRectMake(SpaceBaside * 2, 350, 20, 20)];
    ...
    [argreeButton addTarget:self action:@selector(isArgreeButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:argreeButton];
    
    //是否同意协议
    UILabel *argreeLabel = [[UILabel alloc] initWithFrame:CGRectMake(50, 350, MainScreenWidth - 60, 20)];
    argreeLabel.text = @"同意《云课堂虚拟货币服务协议》";
    ...
    _topUpButton.frame = CGRectMake(50, 400, MainScreenWidth - 100, 40);
    ...
    if (_isArgree == NO) {
        _topUpButton.enabled = NO;
        _topUpButton.backgroundColor = [UIColor groupTableViewBackgroundColor];
    } else {
        _topUpButton.enabled = YES;
        _topUpButton.backgroundColor = BasidColor;
    }
...
{
    if (_moneyField.text.length == 0) {
        [MBProgressHUD showError:@"请选择金额" toView:self.view];
    }else{
        //充值
        if ([_tpyeStr isEqualToString:@"zhifu"]) {//支付宝
            [self BuyNetInfoAlipay];
        } else {//微信
            [self BuyNetInfoWxpay];
        }
    }
}
...
- (void)BuyNetInfoAlipay {
    ...
    [dic setValue:[NSString stringWithFormat:@"%@",_moneyField.text] forKey:@"money"];
