//
//  OnlyClassSearchViewController.m
//  dafengche
//
//  Created by 赛新科技 on 2017/6/7.
...
@interface OnlyClassSearchViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
...
@implementation OnlyClassSearchViewController
...
- (void)addNav {
    ...
    //    //添加搜索
    _searchText = [[UITextField alloc] initWithFrame:CGRectMake(50, 25, MainScreenWidth - 80, 30)];
    _searchText.placeholder = @"搜索科目、老师、机构、课程";
    ...
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 5, 30, 20)];
    [button setBackgroundImage:Image(@"Search") forState:UIControlStateNormal];
...
- (void)addClassTableView {
    ...
    _classTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 142, MainScreenWidth, MainScreenHeight - 142) style:UITableViewStyleGrouped];
...
- (void)addTeacherTableView {
    _teacherTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 142, MainScreenWidth, MainScreenHeight - 142) style:UITableViewStyleGrouped];
...
- (void)addInstationTableView {
    _instationTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 142, MainScreenWidth, MainScreenHeight - 142) style:UITableViewStyleGrouped];
...
- (void)addDownView {
    _downView = [[UIView alloc] initWithFrame:CGRectMake(0, 94, MainScreenWidth, 48)];
    ...
    for (int i = 0 ; i < Num ; i ++) {
        ...
        CGFloat imageW = 20;
        button.imageEdgeInsets =  UIEdgeInsetsMake(0,MainScreenWidth / 6 - imageW / 2,20,MainScreenWidth / 6 - imageW / 2);
        ...
        //添加title
        UILabel *name = [[UILabel alloc] initWithFrame:CGRectMake(0, ButtonH - 20, ButtonW, 20)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_typeStr isEqualToString:@"1"]) {//课程
        ...
        //        static NSString *CellIdentifier = @"cellTeacher";
        //        //自定义cell类
        //        SearchTeacherCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        //        //自定义cell类
        //        if (cell == nil) {
        //            cell = [[SearchTeacherCell alloc] initWithReuseIdentifier:CellIdentifier];
        //        }
        //
        //        NSDictionary *dict = _teacherArray[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_typeStr isEqualToString:@"1"]) {//课程
        ...
        NSString *Cid = _classArray[indexPath.row][@"id"];
        NSString *Price = _classArray[indexPath.row][@"price"];
        NSString *Title = _classArray[indexPath.row][@"video_title"];
        NSString *VideoAddress = _classArray[indexPath.row][@"video_address"];
        NSString *ImageUrl = _classArray[indexPath.row][@"imageurl"];
        ...
        NSString *Cid = [NSString stringWithFormat:@"%@",_classArray[indexPath.row][@"live_id"]];
        NSString *Price = _classArray[indexPath.row][@"price"];
        NSString *Title = _classArray[indexPath.row][@"video_title"];
        NSString *ImageUrl = _classArray[indexPath.row][@"imageurl"];
        ...
        NSString *ID = _teacherArray[indexPath.row][@"id"];
        ...
        instMainVc.schoolID = _instArray[indexPath.row][@"school_id"];
        instMainVc.uID = _instArray[indexPath.row][@"uid"];
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if( scrollView.contentOffset.y > _oldContentY) {
        [UIView animateWithDuration:0.1 animations:^{
            _downView.frame = CGRectMake(0, 64, MainScreenWidth, 48);
            _classTableView.frame = CGRectMake(0, 64 + 48, MainScreenWidth, MainScreenHeight - 64 - 48 + 35);
        }];
    } else {
        [UIView animateWithDuration:0.1 animations:^{
            _downView.frame = CGRectMake(0, 64 + 30, MainScreenWidth, 48);
...
- (void)typeButtonClick:(UIButton *)button {
    ...
    _classType = [NSString stringWithFormat:@"%ld",button.tag + 1];
    NSLog(@"%@",_classType);
...
- (void)rankButton:(UIButton *)button {
    ...
    //    [_allRankButton setTitle:title forState:UIControlStateNormal];
    _allRank.text = title;
...
- (void)netWorkHomeSearch {
    ...
    if (_cate_ID) {
        [dic setObject:_cate_ID forKey:@"cate_id"];
    }
    if (_searchText.text.length > 0) {
        [dic setObject:_searchText.text forKey:@"keyword"];
    }
    if (_orderStr) {
        [dic setObject:_orderStr forKey:@"order"];
    }
...
//            [MBProgressHUD showError:@"没有找到你想要的" toView:self.view];
        }
        
        if (listArray.count == 0) {
            if ([_typeStr integerValue] == 1) {
                //添加空白提示
                if (!_classImageView.image) {
                    ...
                    _classImageView = imageView;
                    [_classTableView addSubview:imageView];
                } else {
                    _classImageView.hidden = NO;
                }
            ...
            if ([_typeStr integerValue] == 1) {
                ...
                _teacherImageView.hidden = YES;
            } else if ([_typeStr integerValue] == 3) {
                _instImageView.hidden = YES;
            } else if ([_typeStr integerValue] == 2) {
                _classImageView.hidden = YES;
            }
        }
        ...
        if ([_typeStr integerValue] == 1) {
            _classArray = [NSMutableArray arrayWithArray:listArray];
            [_classTableView reloadData];
        } else if ([_typeStr integerValue] == 4) {
            _teacherArray = [NSMutableArray arrayWithArray:listArray];
            [_teacherTableView reloadData];
        } else if ([_typeStr integerValue] == 3) {
            _instArray = [NSMutableArray arrayWithArray:listArray];
            [_instationTableView reloadData];
        } else if ([_typeStr integerValue] == 2) {
            _classArray = [NSMutableArray arrayWithArray:listArray];
            [_classTableView reloadData];
...
- (void)netWorkHomeSearchClass {
    ...
    [dic setObject:@"1" forKey:@"page"];
    [dic setObject:@"20" forKey:@"count"];
    if ([_classType integerValue] == 4 || [_classType integerValue] == 3) {
        return;
    } else {
        [dic setObject:_classType forKey:@"type"];
    }
    [dic setObject:_searchText.text forKey:@"keyword"];
    if (_cate_ID) {
        [dic setObject:_cate_ID forKey:@"cate_id"];
