...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_typeStr isEqualToString:@"1"]) {//课程
        ...
        NSString *Cid = [[_classArray objectAtIndex:indexPath.row] stringValueForKey:@"id"];
        ...
        NSString *VideoAddress = [[_classArray objectAtIndex:indexPath.row] stringValueForKey:@"video_address"];
        ...
        if (listArray.count == 0) {
            if ([_typeStr integerValue] == 1) {
                ...
                if (!_teacherImageView.image) {
                    ...
                    _teacherImageView = imageView;
                    [_teacherTableView addSubview:imageView];
                } else {
                    _teacherImageView.hidden = NO;
                }

            } else if ([_typeStr integerValue] == 3) {
                if (!_instImageView.image) {
                    //添加空白提示
                    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 100, 200, 200)];
                    imageView.image = Image(@"更改背景图片.png");
                    imageView.center = CGPointMake(MainScreenWidth / 2 , 300);
                    ...
                    _instImageView = imageView;
                    [_instationTableView addSubview:imageView];
                } else {
                    _instImageView.hidden = NO;
                }
