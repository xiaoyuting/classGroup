...
@interface commentClassVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation commentClassVC

- (id)initWithNumberIds:(NSString *)numberIds
{
    if (self=[super init]) {
        _cateory_id = numberIds;
    }
    return self;
}
...
-(void)requestData:(NSInteger)num
{
    ...
    [dic setValue:@"comtdesc" forKey:@"orderBy"];
    [dic setValue:_num forKey:@"pType"];
    [dic setValue:_cateory_id forKey:@"cateId"];

    NSArray *PLArray = [PLClassTool classWithDic:dic];
    ...
    [manager getClass:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        //保存数据
        if (!PLArray.count) {
            [PLClassTool saveClasses:responseObject[@"data"]];
        }
