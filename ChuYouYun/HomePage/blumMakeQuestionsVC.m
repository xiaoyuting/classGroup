...
@implementation blumMakeQuestionsVC
...
- (void)commitBtn
{
    if (view1.text.length==0) {
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请输入标题" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alter show];
        return;
    }
    else if(view2.text.length==0)
    {
        UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请输入提问内容" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alter show];
        return;
