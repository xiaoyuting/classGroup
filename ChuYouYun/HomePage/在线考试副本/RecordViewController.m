    ...
    if (_dataSourceArray == _singleArray) {//说明是单选题
        ...
        if (_Number == 0) {//说明是多选题的第一题
            ...
            if (_singleArray != nil) {
                ...
                [MBProgressHUD showError:@"已经最前面一题了" toView:self.view];
                return;
            }
        }
...
//            _textView.backgroundColor = [UIColor redColor];
            ...
            NSLog(@"_userZGAnswer----%@",_userZGAnswer);
            if (_userZGAnswer.count >= 2) {
    ...
    if (_dataSourceArray == _singleArray) {//说明是单选题
        ...
        if (_Number == _singleArray.count - 1) {//说明是最后一题了
            ...
            if (_multipleArray != nil) {//多选
                ...
                _Number = _singleArray.count - 1;
                return;
            }
        }
        
    } else if (_dataSourceArray == _multipleArray) {//多选题
        if (_Number == _multipleArray.count - 1) {//说明是最后一题
            ...
            if (_gapArray != nil) {//填空
                ...
                [MBProgressHUD showSuccess:@"已经是最后一题了" toView:self.view];
                _Number = _multipleArray.count - 1;
                return;
            }
        }
        ...
        if (_Number == _gapArray.count - 1) {//说明是最后一题了
            ...
            if (_judgeArray != nil) {//判断
                ...
                [MBProgressHUD showSuccess:@"已经是最后一题了" toView:self.view];
                _Number = _gapArray.count - 1;// 修复最后一题 再点不再试最后一题
                return;
            }
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (_dataSourceArray == _judgeArray) {//判断
        NSString *myStr = _userPDAnswer[_Number];
        NSString *rightStr = _PDRightAnswer[_Number];
    ...
    if (_dataSourceArray == _singleArray) {//单选
        NSString *myStr = _userDXAnswer[_Number];
        NSString *rightStr = _DXRightAnswer[_Number];
        if ([myStr isEqualToString:rightStr]) {
            [falseButton setTitle:@" 答对了" forState:UIControlStateNormal];
            [falseButton setImage:Image(@"考试系统开心@2x") forState:UIControlStateNormal];
        }
    ...
    if (_dataSourceArray == _multipleArray) {//多选
        NSString *myStr = _userDDXAnswer[_Number];
        NSString *rightStr = _DDXRightAnswer[_Number];
        if ([myStr isEqualToString:rightStr]) {
            [falseButton setTitle:@" 答对了" forState:UIControlStateNormal];
            [falseButton setImage:Image(@"考试系统开心@2x") forState:UIControlStateNormal];
        }
...
//        NSString *answerStr = _userDXAnswer[_Number];
//        [_XZArray removeAllObjects];
//        for (int i = 0; i < 20 ; i ++) {
//            [_XZArray addObject:@"0"];
//        }
...
//    } else if ([_typeStr isEqualToString:@"4"]) {//判断
//        
//        NSString *answerStr = _userPDAnswer[_Number];
//        [_XZArray removeAllObjects];
//        for (int i = 0; i < 20 ; i ++) {
//            [_XZArray addObject:@"0"];
//        }
