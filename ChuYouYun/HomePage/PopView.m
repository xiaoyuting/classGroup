...
@interface PopView ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>{
...
-(void)requestData{
    ...
    [manager getpublicPort:nil mod:@"Home" act:@"getCateList" success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [MBProgressHUD showError:@"获取数据失败" toView:self];
    }];
}
...
-(void)initUI{
    ...
    _tableView.frame = CGRectMake(100, 120, _frame.size.width - 100, _frame.size.height);
    _headScrollow.frame = CGRectMake(0, 120, 100, _frame.size.height);
    [view addSubview:_headScrollow];
    [view addSubview:_tableView];
    _headScrollow.backgroundColor = [UIColor redColor];
    //需要传入按钮的个数
...
-(void)addBtn{
    ...
    for (int i =0; i<_dataArray.count; i++) {
        ...
        _leftBtn.titleLabel.font = Font(11);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
   ...
   // if (!cell) {
        ...
        for (int i=0; i<[_cateGorryArr[indexPath.section][@"childs"] count]; i++) {
            ...
            [btn setTitle:_cateGorryArr[indexPath.section][@"childs"][i][@"title"] forState:UIControlStateNormal];
            [btn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
            btn.tag = 1000 + [_cateGorryArr[indexPath.section][@"childs"][i][@"id"] integerValue];
