...
@interface ChatViewController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>
{
    CGRect rect;
    TIXingLable *_txlab;
}
...
- (void)viewDidLoad {
    ...
    //更新消息个数
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(updataChatlist:) name:@"updateMessageCountfdsafdsk231" object:nil];
    
}

//tabbar的轮询发送的通知，判断个数是否有变化，有就刷新列表,只有点赞、评论、at我个数。粉丝等数据
-(void)updataChatlist:(NSNotification *)noti
{
    NSDictionary *dic = noti.object;
//    NSInteger lastAtsCount = _atsCount;
//    NSInteger lastComsCount = _comsCount;
//    NSInteger lastZansCount = _zansCount;
//    _atsCount = [[dic objectForKey:@"atme"] intValue];
//    _comsCount = [[dic objectForKey:@"comment"] intValue];
//    _zansCount = [[dic objectForKey:@"digg"] intValue];
//    if (lastAtsCount!=_atsCount||lastComsCount!=_comsCount||lastZansCount!=_zansCount) {
//        //数据变化，刷新吧，辛巴
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([KKKK isEqualToString:@"3"]) {//总是自己头像
